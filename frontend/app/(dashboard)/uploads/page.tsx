      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 p-4 sm:p-6 md:p-8">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-300 mt-1 sm:mt-2 text-xs sm:text-sm md:text-base">
              Datei-Uploads und Hintergrund-Jobs – Live Daten
            </p>
          </div>
          <div className="inline-flex items-center self-start">
            <input ref={inputRef} type="file" className="hidden" onChange={(e) => onFiles(e.target.files)} />
            <Button
              className="bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600 text-white shadow-lg hover:opacity-90 h-9 sm:h-10 text-xs sm:text-sm"
          ...
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                <a
                  href="/api/uploads/template.csv"
                  target="_blank"
                  className="inline-flex items-center rounded-md border border-white/20 px-3 py-2 text-sm text-slate-200 hover:bg-white/10"
                >
                  CSV Vorlage
                </a>
                <a
                  href="/api/uploads/template.xlsx"
                  target="_blank"
                  className="inline-flex items-center rounded-md border border-white/20 px-3 py-2 text-sm text-slate-200 hover:bg-white/10"
                >
                  Excel Vorlage
          ...
          <CardContent>
              ...
              <table className="w-full text-sm text-slate-300">
                <thead>
                  <tr>
                    {preview.headers.map((h) => (
                      <th key={h} className="px-2 py-1 border-b border-white/10 text-left">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {preview.samples.map((row, idx) => (
                    <tr key={idx}>
                      {preview.headers.map((h) => (
                        <td key={h} className="px-2 py-1 border-b border-white/5">
                          {String(row[h] ?? "")}
                        </td>
                      ))}
                    </tr>
                ...
                <div key={field} className="text-slate-200">
                  <div
                    className={`text-xs mb-1 ${
                      required.includes(field) ? "text-red-300" : "text-slate-400"
                    }`}
                  >
              ...
              <Button
                ...
                onClick={() => {
                  openModal({
                    ...
                    content: <CirclePreview activities={circleActivities} />,
                  })
                }}
              >
                Preview in Circle
            ...
            <div className="mt-6">
              <div className="text-slate-300 mb-2 text-sm">
                Vorschau der Kategorien (aus den ersten Zeilen)
              </div>
                  ...
                  <PieChart>
                    ...
                    >
                      {((preview?.samples || []).length > 0
                        ? Array.from(
                            new Set(
                              (preview?.samples || []).map((r: any) =>
                                String(r[mapping.category || "Category"] || "OTHER").toUpperCase(),
                              ),
                            ),
                          )
                        : []
                        ...
                        <Cell
                          key={String(name) + idx}
                          fill={["#3b82f6", "#a78bfa", "#10b981", "#f59e0b", "#64748b"][idx % 5]}
                        />
                      ))}
                    </Pie>
                    <Tooltip
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6">
        <Card className="bg-white/5 border-white/10 backdrop-blur-xl">
          <CardHeader className="p-3 sm:p-4 pb-1 sm:pb-2">
            <CardTitle className="text-[10px] sm:text-xs md:text-sm text-slate-300 flex items-center gap-1.5 sm:gap-2">
              <HardDrive className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-blue-400" /> Hochgeladen
            </CardTitle>
          </CardHeader>
        ...
        <Card className="bg-white/5 border-white/10 backdrop-blur-xl">
          <CardHeader className="p-3 sm:p-4 pb-1 sm:pb-2">
            <CardTitle className="text-[10px] sm:text-xs md:text-sm text-slate-300 flex items-center gap-1.5 sm:gap-2">
              <Clock3 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-yellow-300" /> Aktiv
            </CardTitle>
          </CardHeader>
          <CardContent className="p-3 sm:p-4 pt-0">
  ...
  return (
    <div className="relative w-full flex items-center justify-center py-10">
      ...
      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 z-10">
        <span className="px-3 py-1 rounded bg-slate-800/70 border border-slate-700 text-slate-200">
          Zoom: {zoom.toFixed(1)}x
        </span>
        <Button
          size="sm"
          variant="outline"
          onClick={() => setZoom((z) => clamp(parseFloat((z - 0.2).toFixed(1))))}
        >
          -
        </Button>
        <Button
      ...
      <div className="relative" style={{ width: base, height: base, overflow: "visible" }}>
        <div
          style={{ width: base, height: base, transform: `scale(${zoom})`, transformOrigin: "center" }}
        >
          <RadialCircle
